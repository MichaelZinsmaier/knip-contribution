...
public class LabelAnnotatorView<T extends RealType<T> & NativeType<T>, L extends Comparable<L>> extends AbstractDefaultAnnotatorView<Labeling<String>>
	...
	protected JComponent createAnnotatorComponent() {
		...
		annotator.addViewerComponent(new LabelingViewInfoPanel<L>());
	...
	protected void currentSelectionChanged(DataCell[] currentRow, int currentColNr, RowColKey key) {
		...
		LabelingCell<L> labelingCell = null;
		
		if (currentRow.length == 2) {
            // Labeling and image
			if (currentRow[0] instanceof ImgPlusValue) {
				imgPlusCell = (ImgPlusCell<T>)currentRow[0];
				labelingCell = (LabelingCell<L>)currentRow[1];
			} else {				
				imgPlusCell = (ImgPlusCell<T>)currentRow[1];
				labelingCell = (LabelingCell<L>)currentRow[0];
			}

			m_eventService.publish(new ImgWithMetadataChgEvent<T>(imgPlusCell.getImgPlus().getImg(), imgPlusCell.getMetadata()));
			m_eventService.publish(new LabelingWithMetadataChgEvent<L>(labelingCell.getLabeling(), labelingCell.getLabelingMetadata()));
        } else {
        	labelingCell = (LabelingCell<L>)currentRow[0];
        	m_eventService.publish(new LabelingWithMetadataChgEvent<L>(labelingCell.getLabeling(), labelingCell.getLabelingMetadata()));
        }
